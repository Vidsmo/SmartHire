...
def test_generate_doc_fallback(client):
    payload = {"type": "offer_letter", "candidate_info": "Test Candidate", "job_info": "Test Role"}
    r = client.post("/generate-doc", json=payload)
    assert r.status_code == 200
    data = r.get_json()
    assert "document" in data and len(data["document"]) > 0
